...
def get_info(ico):
    ...
    while not vr_childs[0].find(text=lambda t: 'Předmět' in t): #making sure we are starting with a child 'předmět podnikání'
        vr_childs = vr_childs[1:]
    keys = []
    space = '    '
    for child in vr_childs:
        test = child.find(class_='aunp-udajPanel') #check if there is any information available, sometimes there are empty childs
        test2 = child.find(text='Předmět podnikání: ')
        if test is None:
            continue
...
def get_vypis():
    ico = str(entry1.get())
    get_vypis_doc(ico)
    label1 = tk.Label(root, text = 'hotovo!')
